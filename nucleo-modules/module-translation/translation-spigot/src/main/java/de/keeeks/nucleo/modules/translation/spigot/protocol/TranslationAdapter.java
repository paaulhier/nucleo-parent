...
public interface TranslationAdapter {
    ...
    default void handleChatComponent(Player player, PacketType type, PacketContainer packet, int index) {
        ...
        if (type == PacketType.Play.Server.SYSTEM_CHAT) {
            var handle = WrappedChatComponent.fromHandle(packet.getStructures().readSafely(0).getHandle());
            WrappedChatComponent translated = translate(player, handle);
            packet.getModifier().write(0, translated.getHandle());
        }
    }
